...
int main() {
    ...
    // Load test image
    Mat img = imread("/wolvi/src/camera_sim/assets/image_01.png");
    int h = img.rows;
    int w = img.cols;

    // Read the values of the camera calibration
    ifstream mtx_file("/wolvi/src/camera_sim/config/camera_matrix_output.txt");
    Mat mtx = parse_camera_matrix(mtx_file);
    mtx_file.close();

    ifstream dits_file("/wolvi/src/camera_sim/config/distortion_coefficients_output.txt");
    ...
    Mat robot_img = imread("/wolvi/src/camera_sim/assets/image_02.png");
